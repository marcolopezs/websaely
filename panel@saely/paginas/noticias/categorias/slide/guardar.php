...
$rst_notgaleria=mysql_query("SELECT * FROM ".$tabla_suf."_noticia_categoria_slide WHERE noticia=$idnoticia", $conexion);
...
if($num_notgaleria>0){
	...
	while($_POST['flash_uploader_'.$cont.'_tmpname']<>""){
		...
		$thumb{$cont}->adaptiveResize(315,240);
		...
		mysql_query("INSERT INTO ".$tabla_suf."_noticia_categoria_slide(fecha, hora, usuario, imagen, noticia, carpeta, orden) VALUES ('$fecha', '$hora', '$usuario_user','$imagen', $idnoticia, '$carpeta', $cont_img)",$conexion);
		$cont++; $cont_img++;
	}
	...
	while($_POST['flash_uploader_'.$cont.'_tmpname']<>""){
		...
		$thumb{$cont}->save("../../../../../imagenes/upload/".$carpeta."thumb/".$_POST['flash_uploader_'.$cont.'_tmpname']."", "jpg");
		$imagen=$_POST['flash_uploader_'.$cont.'_tmpname'];
		mysql_query("INSERT INTO ".$tabla_suf."_noticia_categoria_slide(fecha, hora, usuario, imagen, noticia, carpeta, orden) VALUES ('$fecha', '$hora', '$usuario_user','$imagen', $idnoticia, '$carpeta', $cont)",$conexion);
		$cont++;
	}
}
