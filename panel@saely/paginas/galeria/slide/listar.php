...
//NOTICIA
$rst_noticia=mysql_query("SELECT * FROM ".$tabla_suf."_galeria WHERE id=$idnoticia", $conexion);
$fila_noticia=mysql_fetch_array($rst_noticia);

//GALERIA
$rst_galeria=mysql_query("SELECT * FROM ".$tabla_suf."_galeria_slide WHERE noticia=$idnoticia ORDER BY orden ASC;", $conexion);
...
<ul id="test-list">
	<?php while($fila_galeria=mysql_fetch_array($rst_galeria)){ ?>
        <li id="listItem_<?php echo $fila_galeria["id"] ?>" class="alto">
            ...
            <img src="../../../../imagenes/galeria/<?php echo $fila_galeria["carpeta"]."thumb/".$fila_galeria["imagen"] ?>" width="150" />
        </li>
    <?php } ?>
</ul>
                    </td>
